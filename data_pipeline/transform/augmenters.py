...
from data_pipeline.transform.GridMask import GridMask


def get_affine_augmenter(probs:list):
    random_rotation = T.RandomRotation(degrees=(0, 25))
    random_zoom = T.RandomAffine(degrees=(0, 0), translate=None, scale=(0.5, 1.0), shear=None)
    ...
    rot180 = lambda x: T.functional.rotate(x, angle=180)
    rot270 = lambda x: T.functional.rotate(x, angle=270)
    affine_operations = [
...
def get_quality_augmenter(probs:list):
    ...
    quality_operations = [
        ...
        inverse_color, gaussian_noise, random_sharpness, posterization, grid_mask
    ]
...
def get_hybrid_augmenter(probs:dict):
    affine_augmenter = get_affine_augmenter(probs['affine_aug_probs'])
    quality_augmenter = get_quality_augmenter(probs['quality_aug_probs'])
    zoom_augmenter = get_zoom_augmenter(probs['zoom_aug_probs'])
    def dual(image:Image, probs=probs['hybrid_aug_probs']['dual']):
        ...
        return image
    def triple(image:Image, probs=probs['hybrid_aug_probs']['triple']):
    ...
    hybrid_augmenter = T.RandomChoice(hybrid_operations, p=probs['hybrid_aug_probs']['mode'])
...
def augment_image(image:Image, random_aug_config:dict):
    ...
    hybrid_augmenter = get_hybrid_augmenter(random_aug_config)
    augmenters = [affine_augmenter, quality_augmenter, zoom_augmenter, hybrid_augmenter]
    augmenter = T.RandomChoice(augmenters, p=random_aug_config['aug_probs'])
