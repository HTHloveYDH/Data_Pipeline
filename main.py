...
def main():
    ...
    runtime_global_vars_map.update(
        {'s3_bucket_name': args.s3_bucket_name, 'img_mode': train_config['img_mode']}
    )
    ...
    trainset = dataset_factory.create(training_config['dataset_type']).create_dataset(
        dataset_config['train'], input_size, default_img_size, is_preshuffle, train_transform
        **custom_load_config
    )
    ...
    validset = dataset_factory.create(training_config['dataset_type']).create_dataset(
        dataset_config['valid'], input_size, default_img_size, False, valid_transform
    )
    ...
    testset = dataset_factory.create(training_config['dataset_type']).create_dataset(
        dataset_config['test'], input_size, default_img_size, False, test_transform
    )
